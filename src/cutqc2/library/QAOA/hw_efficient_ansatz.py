...
class HWEA:
    ...
    def gen_circuit(self):
        ...
        if self.parameters == "optimal":
            ...
            raise Exception(f"Unknown parameter option: {self.parameters}")
        ...
        try:
            ...
            for i in range(self.nq):
                # VB 09/25/25: Changed to avoid .u3() (deprecated)
                # self.circ.u3(theta[i + p_idx], 0, 0, self.qr[i])
                self.circ.append(U3Gate(theta[i + p_idx], 0, 0), [self.qr[i]])
            p_idx += self.nq
            ...
            for i in range(self.nq):
                ...
                self.circ.append(U3Gate(0, 0, theta[i + p_idx]), [self.qr[i]])
            p_idx += self.nq
            ...
            raise Exception(f"There was an error in the circuit!. Error = {ex}")
