...
from cutqc2.library.generators import (
    gen_adder,
    gen_BV,
    gen_hwea,
    gen_random,
...
def generate_circ(num_qubits, depth, circuit_type, reg_name="q", connected_only=True, seed=None):
    ...
    while num_trials:
        if circuit_type == "supremacy":
            ...
            full_circ = gen_random(num_qubits, depth, seed)
        else:
            raise Exception("Illegal circuit type:", circuit_type)

        if (full_circ is not None and full_circ.num_tensor_factors() == 1) or (
            full_circ is not None and not connected_only
        ):
            break
        full_circ = None
        num_trials -= 1
